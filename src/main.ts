const validTimePeriods = [
  '1m',
  '3m',
  '5m',
  '15m',
  '30m',
  '1h',
  '2h',
  '4h',
  '6h',
  '12h',
  '1d',
  '3d',
  '1w',
];

const validLocales = ['en', 'es', 'zh-CN', 'de', 'ja-JP', 'el-GR', 'da-DK'];

const requiredColorSchemeKeys = [
  'bg',
  'text',
  'textStrong',
  'textWeak',
  'short',
  'shortFill',
  'long',
  'longFill',
  'cta',
  'ctaHighlight',
  'alert',
];

const defaultOptions = {
  width: '100%',
  height: '100%',
};

function queryToString(query: { [key: string]: string }) {
  let pairs = [];
  for (let key in query) {
    pairs.push(`${key}=${query[key]}`);
  }

  if (pairs.length === 0) {
    return '';
  } else {
    return '?' + pairs.join('&');
  }
}

type TimePeriod =
  | '1m'
  | '3m'
  | '5m'
  | '15m'
  | '30m'
  | '1h'
  | '2h'
  | '4h'
  | '6h'
  | '12h'
  | '1d'
  | '3d'
  | '1w';

type Locale = 'en' | 'es' | 'zh-CN' | 'de' | 'ja-JP' | 'el-GR' | 'da-DK';

interface Options {
  width?: number | string;
  height?: number | string;
  timePeriod?: TimePeriod | null;
  presetColorScheme?: PresetColorScheme;
  locale?: Locale;
  host?: string;
  protocol?: 'http' | 'https';
  query?: { [key: string]: string };
  customColorScheme?: any; // TODO ##############
}

type PresetColorScheme =
  | 'standard'
  | 'candycane'
  | 'albuquerque'
  | 'epaper'
  | 'delek'
  | 'blueprint'
  | 'ballmer'
  | 'bushido'
  | 'ishihara';

export default class Embed {
  exchange: string;
  currencyPair: string;
  opts: Options;

  HTMLElementToAppendTo: HTMLElement | null = null;
  iframeElement: HTMLIFrameElement | null = null;

  constructor(exchange: string, currencyPair: string, opts: Options) {
    this.exchange = exchange;
    this.currencyPair = currencyPair;
    this.opts = {
      ...defaultOptions,
      ...opts,
    };

    // Validate exchange
    if (!exchange) {
      throw new Error('exchange required');
    }

    // Validate currencyPair
    if (!currencyPair) {
      throw new Error('currencyPair required');
    }
    // TODO validate that exchange supports this currencyPair (generated config)
    // As this is now, it will just render an iframe containing the 404 page

    if (!!opts.timePeriod && typeof opts.timePeriod === 'string') {
      if (validTimePeriods.indexOf(opts.timePeriod) === -1) {
        throw new Error(
          `Unknown time period "${
            opts.timePeriod
          }"\nValid timePeriods: ${validTimePeriods.join(', ')}`
        );
      }

      opts.timePeriod = opts.timePeriod.toLowerCase() as TimePeriod;
    } else {
      opts.timePeriod = null; // Will load last selected for returning users, or default (1H) for new users
    }

    if (opts.locale !== undefined) {
      if (validLocales.indexOf(opts.locale) === -1) {
        throw new Error(
          `Unknown locale "${opts.locale}"\nValid locales: ${validLocales.join(
            ', '
          )}`
        );
      }
    }

    if (opts.host === undefined) opts.host = 'embed.cryptowat.ch';
    if (opts.protocol === undefined) opts.protocol = 'https';
    if (opts.width === undefined) opts.width = '100%';
    if (opts.height === undefined) opts.height = '100%';
  }

  get src(): string {
    let path = `/${this.exchange}/${this.currencyPair}`;
    if (this.opts.timePeriod !== null) {
      path += `/${this.opts.timePeriod}`;
    }

    let uri = `${this.opts.protocol}://${this.opts.host}${path}`;
    let query: any = {};
    if (this.opts.locale) {
      query.locale = this.opts.locale;
    }
    if (this.opts.query) {
      // Extend query with additional params that may have been added
      for (let key in this.opts.query) {
        if (query[key] === undefined) query[key] = this.opts.query[key];
      }
    }

    if (this.opts.presetColorScheme !== undefined) {
      query.presetColorScheme = this.opts.presetColorScheme;
    } else if (this.opts.customColorScheme !== undefined) {
      query.customColorScheme = encodeURIComponent(
        JSON.stringify(this.opts.customColorScheme)
      );
    }

    uri += queryToString(query);
    return uri;
  }

  createIframe(): HTMLIFrameElement {
    let iframe = document.createElement('iframe');
    iframe.setAttribute('src', this.src);
    iframe.setAttribute('frameborder', '0');
    iframe.setAttribute('allowfullscreen', 'true');
    iframe.setAttribute('width', `${this.opts.width}`);
    iframe.setAttribute('height', `${this.opts.height}`);

    return iframe;
  }

  mount(elem: HTMLElement | string) {
    if (typeof elem !== 'string' && !(elem instanceof HTMLElement)) {
      throw new Error(
        `Expected an HTML element or a query selector string. Instead got: ${typeof elem}`
      );
    } else if (typeof elem === 'string') {
      this.HTMLElementToAppendTo = document.querySelector(elem);
    }

    if (!this.HTMLElementToAppendTo) throw new Error(`HTML element not found.`);

    this.iframeElement = this.createIframe();
    this.HTMLElementToAppendTo.appendChild(this.iframeElement);
    return elem;
  }

  unmount() {
    if (
      !this.HTMLElementToAppendTo ||
      !(this.HTMLElementToAppendTo instanceof HTMLElement) ||
      !this.iframeElement ||
      !(this.iframeElement instanceof HTMLIFrameElement)
    ) {
      // Fail silently
      return;
    }

    this.HTMLElementToAppendTo.removeChild(this.iframeElement);
    this.iframeElement = null;
    this.HTMLElementToAppendTo = null;
  }
}
